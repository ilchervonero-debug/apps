const CACHE = 'apucore-v11';
const ASSETS = [
  '/apps/apucore/',
  '/apps/apucore/index.html',
  '/apps/apucore/manifest.json',
  '/apps/apucore/icon-192.svg'
];
...
self.addEventListener('fetch', e => {
  ...
  if (isHTML) {
    e.respondWith(fetch(req).then(res => { caches.open(CACHE).then(c => c.put(req, res.clone())); return res; }).catch(() => caches.match(req).then(r => r || caches.match('/apps/apucore/index.html'))));
  } else {
