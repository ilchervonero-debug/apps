const CACHE = 'demoliciones-v1';
const ASSETS = [
  '/apps/demoliciones/',
  '/apps/demoliciones/index.html',
  '/apps/demoliciones/manifest.json',
  '/apps/demoliciones/icon-192.svg'
];
  ...
  if (isHTML) {
    e.respondWith(
      fetch(req).then(res => { caches.open(CACHE).then(c => c.put(req, res.clone())); return res; })
        .catch(() => caches.match(req).then(r => r || caches.match('/apps/demoliciones/index.html')))
    );
  } else {
    e.respondWith(
