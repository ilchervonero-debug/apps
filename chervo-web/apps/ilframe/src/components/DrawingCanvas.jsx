...
import LosaSheet from './LosaSheet'
...
export default function DrawingCanvas() {
  ...
  const losaDraft = useDrawingStore((s) => s.losaDraft)
  ...
  useEffect(() => {
    drawPlan(planRef.current, panels, selectedId, draft, cursor, activeTool, gridMm, view,
      { beams, beamDraft, selectedBeamId, cerchas, cerchaDraft, selectedCerchaId, pilares, selectedPilarId, techos, techoDraft, selectedTechoId, losas, losaDraft, selectedLosaId, tconnects, tcFirst })
    drawElevation(elevRef.current, panels, selectedId, selectedVertex, gridMm, elevView, tconnects,
      { cerchas, selectedCerchaId, pilares, selectedPilarId, techos, selectedTechoId, losas, selectedLosaId })
  }, [panels, beams, beamDraft, selectedBeamId, cerchas, cerchaDraft, selectedCerchaId, pilares, selectedPilarId, techos, techoDraft, selectedTechoId, losas, losaDraft, selectedLosaId, tconnects, tcFirst, selectedId, selectedVertex, draft, cursor, activeTool, gridMm, view, planH, elevH, tab, elevView])

  // al cambiar de panel o cercha, reinicia la vista del alzado (encaja la cara)
  useEffect(() => {
    // eslint-disable-next-line react-hooks/set-state-in-effect
    setElevView({ z: 1, tx: 0, ty: 0 })
  }, [selectedId, selectedCerchaId, selectedPilarId, selectedTechoId, selectedLosaId])
  ...
  const planMove = (e) => {
    ...
    if (dragRef.current.active) {
      ...
      else if (st2.activeTool === 'slab') st2.dragLosa(vbToPlan(worldFromVb(vb)))
  ...
  const planUp = (e) => {
    ...
    if (dragRef.current.active) {
      ...
      else if (st0.activeTool === 'slab') st0.finishLosa()
      else st0.finishWall()
    ...
    if (!lpRef.current.fired && !dragRef.current.moved) {
      ...
      if (['door', 'window', 'opening'].includes(activeTool)) {
        ...
        const losaHit = pickTecho(mm, st.losas) // mismo test punto-en-rect
        if (losaHit) { st.selectLosa(losaHit); st.setLosaSheet(true) }
        ...
        const pilarHit = pickPilar(mm, st.pilares)
        const techoHit = pickTecho(mm, st.techos)
        const losaHit = pickTecho(mm, st.losas)
        if (activeTool === 'viga') {
        ...
        else if (losaHit) { st.selectLosa(losaHit); st.setLosaSheet(true) }
        ...
        <span className="ct-hint">
          {(() => {
            ...
            if (tab === 'plan') {
              ...
              if (activeTool === 'slab') return 'Arrastrá el rectángulo del entrepiso · dirección de viguetas, separación y deck en su hoja'
            ...
            if (selectedLosaId) return `${selectedLosaId} · sección del entrepiso · editá viguetas y deck en su hoja`
            if (!selectedId) return 'Seleccioná un muro, cercha, pilar, techo o losa en la pestaña Planta'
...
function drawPlan(svg, panels, selectedId, draft, cursor, activeTool, gridMm, view, extra) {
  if (!svg) return
  const { beams = [], beamDraft = null, selectedBeamId = null, cerchas = [], cerchaDraft = null, selectedCerchaId = null, pilares = [], selectedPilarId = null, techos = [], techoDraft = null, selectedTechoId = null, losas = [], losaDraft = null, selectedLosaId = null, tconnects = [], tcFirst = null } = extra || {}
  svg.innerHTML = ''
  ...
  // losas (L1…): rectángulo + viguetas en su dirección
  const drawLosaPlan = (lo, isDraft) => {
    const x0 = Math.min(lo.a[0], lo.b[0]), x1 = Math.max(lo.a[0], lo.b[0])
    const y0 = Math.min(lo.a[1], lo.b[1]), y1 = Math.max(lo.a[1], lo.b[1])
    const sel = !isDraft && lo.id === selectedLosaId
    const col = isDraft ? '#fe0000' : (sel ? '#fe0000' : '#888')
    const A = planToVb([x0, y1]), B = planToVb([x1, y0])
    const rx = Math.min(A[0], B[0]), ry = Math.min(A[1], B[1]), rw = Math.abs(B[0] - A[0]), rh = Math.abs(B[1] - A[1])
    svg.appendChild(el('rect', { x: rx, y: ry, width: rw, height: rh, fill: 'rgba(28,28,28,0.04)', stroke: col, 'stroke-width': (sel ? 2 : 1.4) * k, 'stroke-dasharray': isDraft ? `${8 * k} ${4 * k}` : 'none' }))
    if (!isDraft) {
      const dir = lo.dir || 'x', sep = Math.max(200, lo.sep || 400)
      if (dir === 'x') { for (let yy = y0; yy <= y1; yy += sep) { const v = planToVb([x0, yy])[1]; svg.appendChild(el('line', { x1: rx, y1: v, x2: rx + rw, y2: v, stroke: col, 'stroke-width': 1 * k, opacity: 0.7 })) } }
      else { for (let xx = x0; xx <= x1; xx += sep) { const v = planToVb([xx, y0])[0]; svg.appendChild(el('line', { x1: v, y1: ry, x2: v, y2: ry + rh, stroke: col, 'stroke-width': 1 * k, opacity: 0.7 })) } }
      const t2 = el('text', { x: rx + rw / 2, y: ry + rh / 2, 'font-size': 14 * k, fill: col, 'text-anchor': 'middle', 'dominant-baseline': 'middle' })
      t2.textContent = `${lo.id} · ${(Math.abs(x1 - x0) / 1000).toFixed(1)}×${(Math.abs(y1 - y0) / 1000).toFixed(1)}m`
      svg.appendChild(t2)
    }
  }
  losas.forEach((lo) => drawLosaPlan(lo, false))
  if (losaDraft) drawLosaPlan(losaDraft, true)
...
function drawElevation(svg, panels, selectedId, selectedVertex, gridMm, elevView, tconnects, extra) {
  if (!svg) return
  const { cerchas = [], selectedCerchaId = null, pilares = [], selectedPilarId = null, techos = [], selectedTechoId = null, losas = [], selectedLosaId = null } = extra || {}
  svg.innerHTML = ''
  ...
  // si hay una losa seleccionada, el alzado muestra la sección del entrepiso
  const losa = losas.find((x) => x.id === selectedLosaId)
  if (losa) { drawLosaElevation(svg, losa, elevView); return }

  const panel = panels.find((p) => p.id === selectedId)
  if (!panel) {
    const t = el('text', { x: 500, y: ELEV.h / 2, 'font-size': 18, fill: '#bbb', 'text-anchor': 'middle' })
    t.textContent = 'Seleccioná un muro, cercha, pilar, techo o losa en la pestaña Planta'
    svg.appendChild(t)
    return
  }
...
// ── ALZADO de la LOSA / ENTREPISO (sección a nivel Z) ─────────
function drawLosaElevation(svg, losa, elevView) {
  const ev = elevView || { z: 1, tx: 0, ty: 0 }
  const vg = el('g', { transform: `translate(${ev.tx} ${ev.ty}) scale(${ev.z})` })
  svg.appendChild(vg)

  const geo = slabGeometry(losa)
  const span = geo.span
  const canto = slabCanto(losa)
  const nivel = losa.nivel || 2800
  const totalH = nivel + canto
  const marginTop = 96, marginBot = 130, marginX = 90
  const s = Math.min((1000 - marginX * 2) / span, (ELEV.h - marginTop - marginBot) / totalH)
  const ox = (1000 - span * s) / 2
  const oy = marginTop + (ELEV.h - marginTop - marginBot + totalH * s) / 2
  const tx = ([x, y]) => [ox + x * s, oy - y * s]

  // piso (base)
  vg.appendChild(el('line', { x1: tx([0, 0])[0], y1: oy, x2: tx([span, 0])[0], y2: oy, stroke: '#666', 'stroke-width': 1 }))
  // apoyos en los extremos (de piso a cara inferior de la losa)
  ;[0, span].forEach((x) => { const a = tx([x, 0]), b = tx([x, nivel - canto]); vg.appendChild(el('line', { x1: a[0], y1: a[1], x2: b[0], y2: b[1], stroke: '#8a8a8a', 'stroke-width': 1.5 })) })
  // banda del entrepiso (viguetas) — gris, entre nivel-canto y nivel
  const top = tx([0, nivel]), botR = tx([span, nivel - canto])
  vg.appendChild(el('rect', { x: Math.min(top[0], botR[0]), y: Math.min(top[1], botR[1]), width: Math.abs(botR[0] - top[0]), height: Math.abs(botR[1] - top[1]), fill: 'rgba(28,28,28,0.06)', stroke: '#8a8a8a', 'stroke-width': 1.5 }))
  // deck (placa) arriba, en rojo
  const d0 = tx([0, nivel]), d1 = tx([span, nivel])
  vg.appendChild(el('line', { x1: d0[0], y1: d0[1], x2: d1[0], y2: d1[1], stroke: '#fe0000', 'stroke-width': 3, 'stroke-linecap': 'round' }))

  const at = el('text', { x: 500, y: top[1] - 12, 'font-size': 16, fill: '#444', 'text-anchor': 'middle' })
  at.textContent = `nivel ${(nivel / 1000).toFixed(2)} m · canto ${(canto / 10).toFixed(1)} cm`
  vg.appendChild(at)
  const lz = el('text', { x: 500, y: oy + 26, 'font-size': 16, fill: '#444', 'text-anchor': 'middle' })
  lz.textContent = `luz ${(span / 1000).toFixed(2)} m · ${geo.count} viguetas`
  vg.appendChild(lz)
  const ti = el('text', { x: 500, y: 40, 'font-size': 16, fill: '#999', 'text-anchor': 'middle' })
  ti.textContent = `${losa.id} · entrepiso`
  vg.appendChild(ti)
  geo.alerts.forEach((a, i) => { const t = el('text', { x: 500, y: 62 + i * 20, 'font-size': 14, fill: '#fe0000', 'text-anchor': 'middle' }); t.textContent = a; vg.appendChild(t) })
}
